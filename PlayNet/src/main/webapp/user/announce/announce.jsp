...
<div class="wrap">
	...
	<div id="contents2" style=" color:white;">	
		...
		<div class="Search">
			<form name="frm" method="POST" action="${ path }/user/announce.go">
				<table>
					<tr>
						<th>제목</th>
				...
				<tbody style="text-align:center;">	
					...
					for(AnnounceVo anv : aListTf){%>
						<tr>
							...
							<td style="cursor:pointer;" onclick="javascript:location.href='${ path }/user/announceView.ok?anidx=<%=anv.getAnidx()%>&istop=Y'">
								<%=anv.getTitle()%>
							</td>
							<td>
								<%=anv.getWriter()%>
					...
					<%for(AnnounceVo anv : aList){%>
						<tr>
							...
							<td style="cursor:pointer;" onclick="javascript:location.href='${ path }/user/announceView.ok?anidx=<%=anv.getAnidx()%>'">
								<%=anv.getTitle()%>
							</td>
							<td>
								<%=anv.getWriter()%>
						...
						<td colspan='5'>
							<div id="paging" style="text-align:center;">
								<ol style="list-style:none;">
									<li style="cursor:pointer;" onclick="javascript:location.href='${ path }/user/announce.go?pageNum=1'">처음으로</li>
									<%
									for(int i = pageStartNum; i <= pageEndNum; i++){%>
										<li style="cursor:pointer;" onclick="javascript:location.href='${ path }/user/announce.go?pageNum=<%=pageNum%>'"><%=i%></li>
									<%}%>
									<li style="cursor:pointer;" onclick="javascript:location.href='${ path }/user/announce.go?pageNum=<%=totalPageNum%>'">마지막으로</li>
								</ol>
							</div>
						</td>
					</tr>	
