$(function() {		
		...
		$("select[name=cate1]").change(() => {			
			let pCateIdx = $("select[name=cate1]").find("option:selected").val();
		...
		$("#AddFileBtn").click((e) => {
			if(fileCnt <= 5){
				$(e.target).parent().append("<input style='display:block;' type='file' name='image"+fileCnt+ "'>");
				fileCnt++;
			}
		});
		...
		$("#RemoveFileBtn").click(() => {
			if(fileCnt > 2){
				$("#RemoveFileBtn").parent().find("input:file").filter(":last").remove();
				fileCnt--;
			}
		});
		...
		$("#frm").submit(() => {
			let title = $("#frm").find("input[name=title]");
